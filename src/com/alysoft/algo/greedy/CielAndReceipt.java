...
public class CielAndReceipt
{
	...
	{
		int[] menuArr = new int[12];
		fillMenuValues(menuArr);
		Scanner sc = new Scanner(System.in);
		int t = sc.nextInt();
		while (t > 0)
		{
			int n = sc.nextInt();
			int result = getMenuCount(menuArr, n);
			System.out.println(result);
		}
	}

	private static int getMenuCount(int[] menuArr, int n)
	{
		int i = menuArr.length-1;
		int noOfMenuItems = 0;
		while (i >= 0) {
			if (n <=0) {
				break;
			}
			int menuVal = menuArr[i];
			if (menuVal <= n) {
				n = n-menuVal;
				noOfMenuItems++;
			} else {
	...
	private static void fillMenuValues(int[] menuArr)
	{
		int n = menuArr.length;
		for (int i = 0; i<n; i++) {
			menuArr[i] = (int)Math.pow(2, i);
		}
	}
}
