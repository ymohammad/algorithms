...
3
Explanation
Example case 1. : Remove 1 stone from each of the piles. Now 0 stones are remaining, 
so chef cannot remove any more stones from the piles. Hence, answer is 0+0 = 0

Example case 2. : Again, remove 1 stone from both the piles to get (0,1) stones. 
Now chef cannot remove any more stones from pile 1, so he stops. Hence, answer is 0+1 = 1.

Example case 3. : First remove 1 stone from both the piles to get (3,4) stones. 
Now, remove 2 stones from both the piles so that (1,2) stones are remaining. 
Now chef cannot remove any more stones owing to the condition that he cannot remove the same number of stones twice. So, the answer is 1+2 = 3.
 * @author ymohammad
 *
 */
public class ChefAndTheStones
{
	public static void main(String[] args) throws Exception {
		try
		{
			...
			while (t > 0)
			{
				...
				long result = getStoneCount(n1, n2, m);
	...
	private static long getStoneCount(long n1, long n2, long m)
	{
		long returnResult = 0l;
		long x = (m*(m+1))/2;
		returnResult = (n1<x || n2 < x) ? Math.abs(n2-n1) : ((n1-x) + (n2-x));
		return returnResult;
	}
}
